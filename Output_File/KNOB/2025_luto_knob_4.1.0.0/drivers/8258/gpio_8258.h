...
#ifdef  GPIO_DBG_EN
	#define DBG_GPIO_SEND_STR(x)   Dbg_sendString((s8*)x)
	#define DBG_GPIO_SEND_INT(x)   Dbg_sendInt(x)
	#define DBG_GPIO_SEND_HEX(x)   Dbg_sendHex(x)
	#define DBG_GPIO_SEND_BYTE(x)  Dbg_sendOneByteHex(x)
#else
	#define DBG_GPIO_SEND_STR(x)
	#define DBG_GPIO_SEND_INT(x)
	#define DBG_GPIO_SEND_HEX(x)
	#define DBG_GPIO_SEND_BYTE(x)
#endif
...
typedef enum{
	    ...
	    GPIO_PA0 = GPIO_GROUPA | BIT(0),	GPIO_PWM0NA0=GPIO_PA0,  GPIO_URXA0=GPIO_PA0,
...
static inline bool gpio_read_output(GPIO_PinTypeDef pin)
{
	return (BM_IS_SET(reg_gpio_out(pin), pin & 0xff)?1:0);
}
