...
typeCommon_HandleRxMessageFunc   pvCommon_HandleRxMessage = NULL;

#include "../mesh/user/debug.h"
#ifdef MESH_COMMON_DBG_EN
#define DBG_MESH_COMMON_SEND_STR(x)   Dbg_sendString((s8*)x)
#define DBG_MESH_COMMON_SEND_INT(x)   Dbg_sendInt(x)
#define DBG_MESH_COMMON_SEND_HEX(x)   Dbg_sendHex(x)
#else
#define DBG_MESH_COMMON_SEND_STR(x)
#define DBG_MESH_COMMON_SEND_INT(x)
#define DBG_MESH_COMMON_SEND_HEX(x)
#endif
...
void beacon_str_init()
{
	...
	beacon_send.en = 1;
	beacon_send.inter =
			MAX_BEACON_SEND_INTERVAL + rand()%MAX_BEACON_SEND_INTERVAL;;
	return ;
}
...
#if MD_SERVER_EN
u32 publish_status_periodically_ms = 0;
...
void handle_rx_message_callback_init(typeCommon_HandleRxMessageFunc pCallback)
{
	if(pCallback != NULL)
	{
		pvCommon_HandleRxMessage = pCallback;
	}
}

int app_hci_cmd_from_usb_handle (u8 *buff, int n) // for both usb and uart
{
	if(pvCommon_HandleRxMessage != NULL)
	{
		pvCommon_HandleRxMessage(buff, n);
	}
	return 0;
}
...
#if(HCI_LOG_FW_EN)
_align_4_ u8 uart_hw_tx_buf[HCI_TX_FIFO_SIZE]; // not for user
#else
...
u8 get_ui_ota_is_working(void)
{
	return ui_ota_is_working;
}
