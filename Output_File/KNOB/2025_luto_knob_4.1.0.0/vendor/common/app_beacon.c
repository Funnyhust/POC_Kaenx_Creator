...
#ifdef APP_BEACON_DBG_EN
#define DBG_APP_BEACON_SEND_STR(x)   Dbg_sendString((s8*)x)
#define DBG_APP_BEACON_SEND_INT(x)   Dbg_sendInt(x)
#define DBG_APP_BEACON_SEND_HEX(x)   Dbg_sendHex(x)
#else
#define DBG_APP_BEACON_SEND_STR(x)
#define DBG_APP_BEACON_SEND_INT(x)
#define DBG_APP_BEACON_SEND_HEX(x)
#endif
...
int unprov_beacon_send(u8 mode ,u8 blt_sts)
{
	...
	if(blt_sts){
		...
		DBG_APP_BEACON_SEND_STR("\n mesh_tx_cmd_add_packet");
		beaconData.trans_par_val &= 0xF8;
