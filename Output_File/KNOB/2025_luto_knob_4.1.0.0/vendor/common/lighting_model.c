...
{
	return knob_send_request_message_to_mcu(LIGHTNESS_GET, cb_par->model_idx, par, par_len, true);
}
...
{
	return knob_handle_control_message(par, par_len, cb_par, LIGHTNESS_SET);
}
...
/**
 * @func    light_ctl_temp_prensent_get
 * @brief   None
 * @param
 * @retval
 */
...
/**
 * @func    light_ctl_temp_target_get
 * @brief   None
 * @param
 * @retval
 */
...
void mesh_light_ctl_st_rsp_par_fill(mesh_cmd_light_ctl_st_t *rsp, u8 idx)
{
	...
	if(ct_flag){
	    ...
	    rsp->present_lightness = 0;
	    rsp->target_lightness = 0;
	}
#else
	rsp->present_lightness = get_lightness_from_level(level_st.present_level);
	rsp->target_lightness = get_lightness_from_level(level_st.target_level);
...
{
	return knob_send_request_message_to_mcu(LIGHT_CTL_GET, cb_par->model_idx, par, par_len, true);
}
