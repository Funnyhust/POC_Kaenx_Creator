...
#include "vendor/mesh/user/sw_binding.h"
...
#ifdef  SCENE_DBG_EN
#define DBG_SCENE_SEND_STR(x)   Dbg_sendString((s8*)x)
#define DBG_SCENE_SEND_INT(x)   Dbg_sendInt(x)
#define DBG_SCENE_SEND_HEX(x)   Dbg_sendHex(x)
#define DBG_SCENE_SEND_BYTE(x)  Dbg_sendHexOneByte(x)
#else
#define DBG_SCENE_SEND_STR(x)
#define DBG_SCENE_SEND_INT(x)
#define DBG_SCENE_SEND_HEX(x)
#define DBG_SCENE_SEND_BYTE(x)
#endif
    ...
    if((SCENE_STORE == op)||(SCENE_STORE_NOACK == op)){
		...
		if(p_save) {
			...
			if(type == TYPE_SWITCH) {
				...
				p_save->level = curtain_par->level[cb_par->model_idx];
			}
			else if(type == TYPE_FAN) {
				p_save->level = knob_device_get_host_fan_level(cb_par->model_idx);
				DBG_SCENE_SEND_STR("\n SCENE - FAN");
			}
    	...
    	foreach(i,SCENE_CNT_MAX){
    		...
    		if(p_recall->id == p->id){
				...
				if(type == TYPE_SWITCH) {
					...
					onoff_set.onoff = st;
					knob_handle_mesh_cmd_sig_g_on_off_set((u8*)&onoff_set, sizeof(mesh_cmd_g_onoff_set_t), cb_par);
				}
				else if(type == TYPE_CCT) {
					mesh_cmd_light_ctl_set_t mesh_cmd_light_ctl_set;
					mesh_cmd_light_ctl_set.lightness = s16_to_u16(p->lightness_s16);
					mesh_cmd_light_ctl_set.temp = get_ctl_temp_from_level(p->temp_s16, CTL_TEMP_MIN, CTL_TEMP_MAX);
					mesh_cmd_light_ctl_set.delta_uv = s16_to_u16(p->delta_uv_s16);
					knob_handle_control_message((u8*)&mesh_cmd_light_ctl_set, sizeof(mesh_cmd_light_ctl_set_t), cb_par, LIGHT_CTL_SET);
				}
				else if(type == TYPE_DIM) {
					mesh_cmd_lightness_set_t mesh_cmd_lightness_set;
					mesh_cmd_lightness_set.lightness = s16_to_u16(p->lightness_s16);
					knob_handle_control_message((u8*)&mesh_cmd_lightness_set, 6, cb_par, LIGHTNESS_SET);
					DBG_SCENE_SEND_STR("\n LIGHTNESS_SET - DIM");
				}
				else if(type == TYPE_CURTAIN) {
					vd_cmd_g_level_set_t level_set;
					level_set.level = p->level;
					level_set.type = CUR_CMD_RUN;
					knob_handle_control_message((u8*)&level_set, sizeof(vd_cmd_g_level_set_t), cb_par, G_LEVEL_SET);
				}
				else if(type == TYPE_FAN) {
					vd_cmd_g_level_set_t level_set;
					level_set.level = p->level;
					level_set.type = FAN_CMD_LEVEL;
					knob_handle_control_message((u8*)&level_set, sizeof(vd_cmd_g_level_set_t), cb_par, G_LEVEL_SET);
				}
				else {
...
/**
 * @func    scene_callback_init
 * @brief
 * @param
 * @retval  None
 */
void scene_callback_init(type_handle_get_level_callback_func callbackFunc)
{
	if(callbackFunc != NULL) {
		pv_handle_get_level = callbackFunc;
	}
}

/**
 * @func    scene_rsp_delay_callback_init
 * @brief
 * @param
 * @retval  None
 */
void scene_rsp_delay_callback_init(typeSCENE_setUpSceneRegisterRspDelay func)
{
	if(func != NULL) {
...
/**
 * @func    scene_delete_all_scene_manual
 * @brief
 * @param
 * @retval  None
 */
bool scene_delete_all_scene_manual(u8 model_idx)
{
	...
	bool save = false;
	foreach(i, SCENE_CNT_MAX){
		scene_data_t *p = model_sig_scene.data[model_idx];
		if(p->id != 0) {
			memset(p, 0, sizeof(scene_data_t));
			save = true;
		}
