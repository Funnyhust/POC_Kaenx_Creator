...
#include "vendor/mesh/user/bind.h"

#include "../mesh/user/debug.h"
#ifdef SYTEM_TIME_DBG_EN
	#define DBG_SYTEM_TIME_SEND_STR(x)   Dbg_sendString((s8*)x)
	#define DBG_SYTEM_TIME_SEND_INT(x)   Dbg_sendInt(x)
	#define DBG_SYTEM_TIME_SEND_HEX(x)   Dbg_sendHex(x)
	#define DBG_SYTEM_TIME_SEND_BYTE(x)  Dbg_sendOneByteHex(x)
#else
	#define DBG_SYTEM_TIME_SEND_STR(x)
	#define DBG_SYTEM_TIME_SEND_INT(x)
	#define DBG_SYTEM_TIME_SEND_HEX(x)
	#define DBG_SYTEM_TIME_SEND_BYTE(x)
#endif

type_handleUpdateStatePeriodically pv_handleUpdateStateperiodically = NULL;
type_funcRun100msInterval          pv_userFuncRun100msInterval      = NULL;
...
void system_timer_handle_100ms()
{
	...
	user_ali_time_proc();
    #endif
	if(pv_userFuncRun100msInterval != NULL) {
		pv_userFuncRun100msInterval();
	}
}
...
void system_time_run(){
	...
	#endif
	{
    	...
    	if(get_provision_state() == STATE_DEV_PROVED) {
    		if(pv_handleUpdateStateperiodically != NULL) {
    			pv_handleUpdateStateperiodically();
    		}
    	}
	    #endif
    #endif
...
void system_time_callback_init(
		type_handleUpdateStatePeriodically callback_func, type_funcRun100msInterval callback_func_1)
{
	if(callback_func != NULL){
		pv_handleUpdateStateperiodically = callback_func;
	}
	if(callback_func_1 != NULL) {
		pv_userFuncRun100msInterval = callback_func_1;
	}
}
 ...
 * @param  DWORD_P pWTimerStartTick
 * @retval None
 */
void TIMER_Reset(u32 *pWTimerStartTick)
{
    *pWTimerStartTick = clock_time_ms();
}
