...
#if LOW_POWER
#define LUMEN_MAP_INDEX_MAX      MAX_DIM_STEP
#define LUMEN_MAP_MAX            rgb_lumen_map[MAX_DIM_STEP]
#define TEMP_ERROR_IN_RANGE      20
//#define LIGHTNESS_MIN_2_LIGHT    43909
#define LIGHTNESS_MIN_2_LIGHT    42432  // 65%
#endif

#define DIM_STEP_100       0x0
#define DIM_STEP_375       0x1
#define DIM_STEP_500       0x2
#define DIM_STEP_CHANGE    0x10

/*
#define LIGHT_DIM_STEP    DIM_STEP_500
#define MAX_DIM_STEP      500
*/
#define LIGHT_DIM_STEP    DIM_STEP_CHANGE
#define MAX_DIM_STEP      (ARRAY_SIZE(rgb_lumen_map) - 1)

#ifdef _INVERT_OUTPUT_
	#define LED_INDICATE_VAL    (rgb_lumen_map[0])
#else
	#if LOW_POWER
	#define LED_INDICATE_VAL    (rgb_lumen_map[MAX_DIM_STEP] >> 1)
	#else
	#define LED_INDICATE_VAL    (rgb_lumen_map[MAX_DIM_STEP])
	#endif
#endif
...
#if PWM_HIGHT_SPEED
	#define PWM_FREQ	        (8000)   // Confirm later
#else
	#define PWM_FREQ	        (2000)   // unit: Hz
#endif
...
enum LightMode_e{
	LIGHT_NORMAL_MODE      = 0,
	LIGHT_AUTO_CHANGE_MODE = 1
};
typedef uint8_t LightMode_e;
...
typedef struct {
	...
	u8  flag;
}publish_status_delay_t;


#define ADR_GROUP_START_POINT        0xC000

#define DEFAULT_PUBLISH_DELAY_TIME   100
#define PUBLISH_DELAY_OFFSET         50

#define US_DELAY_TIME_TO_SAVE_LIGHT_STATE    3000000       // 3s

#define MIN_RGB_VAL_WHEN_CAL_HUE     2
...
#if (LIGHT_DIM_STEP == DIM_STEP_100)
...
#define LGT_CMD_SUC_ADD_APPKEY          0xca
#define LGT_CMD_WAIT_POWER_OFF_ON_RESET 0xcb
#define LGT_CMD_CONFIG_SCENE            0xcc
#define LGT_CMD_JOIN_NEW_NET 			0xcd
#define LGT_CMD_WR_TEN_SECOND           0xce
#define LGT_CMD_RESET_BY_BUTTON         0xcf

#define LGT_CMD_SWITCH_POWERON 			0xd0
#define LGT_CMD_SWITCH_PROVISION		0xd1
#define LGT_CMD_SWITCH_CMD		        0xd2

#define LGT_CMD_CONFIG_TIME             0xd3
...
#define PWM_FUNC_W  AS_PWM
	...
	#define LIGHT_WARM_IDX   0
	#define LIGHT_WHITE_IDX  1
#elif LIGHT_TYPE_SEL == LIGHT_TYPE_HSL
	#define LIGHT_RED_IDX    0
	#define LIGHT_GREEN_IDX  1
	#define LIGHT_BLUE_IDX   3
#else
	#define LIGHT_RED_IDX    0
	#define LIGHT_GREEN_IDX  1
	#define LIGHT_BLUE_IDX   2
	#define LIGHT_WARM_IDX   3
    #define LIGHT_WHITE_IDX  4
#endif
...
#else
u32 get_pwm_cmp(u16 lum);
#endif

void light_change_value_with_step(u8 src, u8* dst, u8 step);
u8 light_get_ct_flag(void);

#if LIGHT_TYPE_HSL_EN
void light_manual_change_present_hsl_by_rgb_code(rgbww_t* rgbww);
int get_led_flash_finish(void);
#endif
...
typedef struct {
	...
	u16 step;
	u8  trans_t_ms;
}sig_up_down_control_t;

#define ENDPOINT_INDEX_SUPPORT_RGB    (0x0)
#define ENDPOINT_INDEX_SUPPORT_WW     (0x1)
#define ENDPOINT_ROOT                 (0x0)

#if TEMP_ALLOW_LIGHTNESS_EN && PROX_SS_ENABLE
extern u8 control_manual_temp;
extern u8 enable_offset_time;
#endif
int light_dim_control_manual(u16 adr_dst, u8 idx, sig_up_down_control_t* control_par);
void light_dim_set_hw_by_pwm_idx(int idx, u16 val);
void light_sub_pid_indicate(u8 sub_pid);
void cfg_led_event (u32 e);
void vd_dim_control_init();
void light_set_default_color_when_power_up(u8 idx);
void light_dim_sync(void);
...
#if MD_LEVEL_EN
typedef void (*type_handle_send_dev_st_callback_func)(u8);
