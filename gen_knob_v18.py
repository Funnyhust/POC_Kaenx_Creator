# Generator knob_V18
# Changes from v17:
# 1. Added Fan Steps (2/3/4) and Fan Auto toggle for AC endpoint
# 2. Added Swing Positions (Off/OnOff/P0~P5) for AC endpoint
# 3. Fixed Swing DPT from 5.001 to 5.010
...
add_pt_enum("AcFanSteps", "Fan Speed Steps", {2: "2 Speeds (Low/High)", 3: "3 Speeds (Low/Mid/High)", 4: "4 Speeds (Auto/Low/Mid/High)"})
add_pt_enum("AcSwingPos", "Swing Positions", {0: "Not Supported", 1: "On/Off only", 6: "6 Positions (Auto+P1~P4+Stop)", 7: "7 Positions (Auto+P1~P5+Stop)"})
...
for ep in range(1, 7):
    ...
    # AC Mode toggles
    _, rid_ac_en_auto = add_param_basic(f"Btn{ep}_AcEnAuto", "YesNo", "Support Auto Mode", "1", 1)
    _, rid_ac_en_heat = add_param_basic(f"Btn{ep}_AcEnHeat", "YesNo", "Support Heat Mode", "1", 1)
    _, rid_ac_en_cool = add_param_basic(f"Btn{ep}_AcEnCool", "YesNo", "Support Cool Mode", "1", 1)
    _, rid_ac_en_dry = add_param_basic(f"Btn{ep}_AcEnDry", "YesNo", "Support Dry Mode", "1", 1)
    _, rid_ac_en_fan = add_param_basic(f"Btn{ep}_AcEnFan", "YesNo", "Support Fan Mode", "1", 1)
    # AC Fan Speed
    _, rid_ac_fan_steps = add_param_basic(f"Btn{ep}_AcFanSteps", "AcFanSteps", "Fan Speed Steps", "3", 1)
    _, rid_ac_fan_auto = add_param_basic(f"Btn{ep}_AcFanAuto", "YesNo", "Support Fan Auto", "1", 1)
    # AC Swing
    _, rid_ac_swing_pos = add_param_basic(f"Btn{ep}_AcSwingPos", "AcSwingPos", "Swing Positions", "6", 1)
    ac_objs = [
        ...
        create_obj_struct(f"Btn{ep}_AcSwi", "Swing", "1 Byte", "DPST-5-10", ep_ui),
        ...
        create_obj_struct(f"Btn{ep}_AcSwiS", "Swing Status", "1 Byte", "DPST-5-10", ep_ui, is_status=True)
    ]
    ...
    endpoints_logic.append({
        ...
        "fan": {"objs": fan_objs},
        "ac": {"rid_step": rid_ac_step, "rid_en_auto": rid_ac_en_auto, "rid_en_heat": rid_ac_en_heat, "rid_en_cool": rid_ac_en_cool, "rid_en_dry": rid_ac_en_dry, "rid_en_fan": rid_ac_en_fan, "rid_fan_steps": rid_ac_fan_steps, "rid_fan_auto": rid_ac_fan_auto, "rid_swing_pos": rid_ac_swing_pos, "objs": ac_objs},
        "th": {"objs": th_objs},
    })
...
def render_endpoint_block(cfg, indent):
    ...
    if ep_n == 1:
        ...
        res.append(f'{I}    <ParameterRefRef RefId="{cfg["ac"]["rid_fan_steps"]}" />')
        ...
        res.append(f'{I}    <ParameterRefRef RefId="{cfg["ac"]["rid_swing_pos"]}" />')
...
with open(output_path, "w", encoding="utf-8") as f:
    f.write("\n".join(lines) + "\n")
print(f"Generated knob_device.xml v18: Added Fan Steps, Fan Auto, Swing Positions.")
