...
mem_offset = 0
def add_param_basic(internal_name, pt_name, display_text, default_val, obj_size_bytes=1):
    global mem_offset
    pid, rid = new_p_id()
...
add_pt_enum("EndpointFunction", "Endpoint Function", {1: "Switch", 2: "CCT", 3: "Dimming", 4: "Curtain", 5: "Fan", 6: "AC", 7: "Thermostat", 8: "Scene control"})
...
add_pt_enum("SceneAction", "Scene Action", {1: "Recall scene", 2: "Store scene"})

# Global Params
_rid_ep_count = add_param_basic("Gen_EpCount", "EndpointCount", "Endpoint Count", "1", 1)[1]
_rid_bright = add_param_basic("Gen_Brightness", "Percent", "Screen Brightness", "80", 1)[1]
_rid_sleep = add_param_basic("Gen_SleepTime", "SleepTime", "Turn Off Screen After", "300", 2)[1]
...
for sc in range(1, 17):
    # EMPTY default names as requested
...
for ep in range(1, 7):
    ...
    # 8. Scene Control (NEW)
    _, rid_sc_act = add_param_basic(f"Btn{ep}_ScAct", "SceneAction", "Action", "1", 1)
    _, rid_sc_num = add_param_basic(f"Btn{ep}_ScNum", "SceneNumber", "Scene Number", "1", 1)
    sc_objs = [
        create_obj_struct(f"Btn{ep}_Scene", "Scene control", "1 Byte", "DPST-18-1", ep_ui)
    ]
    ...
    endpoints_logic.append({
        ...
        "th": {"objs": th_objs},
        "sc": {"rid_act": rid_sc_act, "rid_num": rid_sc_num, "objs": sc_objs}
    })
...
for cfg in endpoints_logic:
    ...
    if ep_n == 1:
        ...
        L(f'                <ParameterSeparator Id="{new_pb_id()}" Text="Fixed Function: Switch" />')
        ...
        L('                  <when test="1"><ParameterRefRef RefId="%s" /></when>' % cfg["sw"]["rid_startup"])
        L('                  <when test="2">')
        ...
        L('                      </when>')
        L('                      <when test="6">') # AC
        L(f'                        <ParameterRefRef RefId="{cfg["ac"]["rid_step"]}" />')
        ...
        L('                      </when>')
        L('                      <when test="8">') # Scene control
        for orid in cfg["sc"]["orids"]: L(f'                        <ComObjectRefRef RefId="{orid}" />')
        L(f'                        <ParameterRefRef RefId="{cfg["sc"]["rid_act"]}" />')
...
for sc, rid_n, rid_i, rid_num in scene_defs:
    # FIXED Choose logic: Added <when> tag
    L(f'                    <choose ParamRefId="{_rid_sc_count}">')
    L(f'                      <when test="{">=" if sc > 1 else ""}{sc}">')
...
with open(output_path, "w", encoding="utf-8") as f:
    f.write("\n".join(lines) + "\n")
print("Generated knob_device.xml v14: Fixed scene visibility, cleared names, added Scene control function")
