# Generator knob_V20
# Changes from v19:
# 1. Removed "Enable scene" option from General settings completely
# 2. Endpoint count always allows up to 6 (no 5-endpoint restriction)
# 3. Added "Scene" (value=8) to EndpointFunction enum for all 6 endpoints
# 4. Scene settings block is now always visible (not gated by Enable Scene)
# 5. Added mutual exclusion logic: when one endpoint selects Scene, the others
#    cannot select Scene (they see a reduced EndpointFunction list without Scene)
...
# ParameterTypes
# Single endpoint count type: always 1-6
...
# Full endpoint function list including Scene (value=8)
add_pt_enum("EndpointFunction", "Endpoint function",
    {0: "Disabled", 1: "Switch", 2: "CCT", 3: "Dimmer", 4: "Curtain", 5: "Fan", 6: "Air conditioner", 7: "Heater", 8: "Scene control"})
# Restricted endpoint function list WITHOUT Scene control (for endpoints that can't use Scene)
add_pt_enum("EndpointFunctionNoScene", "Endpoint function (no scene)",
    {0: "Disabled", 1: "Switch", 2: "CCT", 3: "Dimmer", 4: "Curtain", 5: "Fan", 6: "Air conditioner", 7: "Heater"})
...
# Global Params
# NOTE: Gen_EnableScene parameter is REMOVED (no longer needed)
...
endpoints_logic = []
# Store all rid_func refs for mutual exclusion logic
ep_rid_funcs = []
...
for ep in range(1, 7):
    ...
    # He byte params
    _, rid_he_step     = add_param_basic(f"Btn{ep}_HeStep", "AcStep", "  Setpoint step", "0", 1)
    _, rid_he_min_temp = add_param_basic(f"Btn{ep}_HeMinTemp", "AcTempMin", "  Min temperature", "16", 1)
    _, rid_he_max_temp = add_param_basic(f"Btn{ep}_HeMaxTemp", "AcTempMax", "  Max temperature", "30", 1)
    ...
    # GO Text uses function-specific names so the GO table shows correct names (#2)
    _sw_ui  = f"Endpoint {ep} - Switch"
    _cct_ui = f"Endpoint {ep} - CCT"
    _dim_ui = f"Endpoint {ep} - Dimmer"
    _cu_ui  = f"Endpoint {ep} - Curtain"
    _fan_ui = f"Endpoint {ep} - Fan"
    _ac_ui  = f"Endpoint {ep} - Air conditioner"
    _he_ui  = f"Endpoint {ep} - Heater"
    _sc_ui  = f"Endpoint {ep} - Scene control"
    sw_objs = [
        create_obj_struct(f"Btn{ep}_Switch", "Switch", "1 Bit", "DPST-1-1", _sw_ui),
        create_obj_struct(f"Btn{ep}_Stat", "Switch status", "1 Bit", "DPST-1-1", _sw_ui, is_status=True)
    ]
    cct_objs = [
        create_obj_struct(f"Btn{ep}_CctPow", "Switch", "1 Bit", "DPST-1-1", _cct_ui),
        create_obj_struct(f"Btn{ep}_CctBri", "Brightness", "1 Byte", "DPST-5-1", _cct_ui),
        create_obj_struct(f"Btn{ep}_CctCol", "Color temperature", "2 Bytes", "DPST-7-600", _cct_ui),
        create_obj_struct(f"Btn{ep}_CctPowS", "Switch status", "1 Bit", "DPST-1-1", _cct_ui, is_status=True),
        create_obj_struct(f"Btn{ep}_CctBriS", "Brightness status", "1 Byte", "DPST-5-1", _cct_ui, is_status=True),
        create_obj_struct(f"Btn{ep}_CctColS", "Color temperature status", "2 Bytes", "DPST-7-600", _cct_ui, is_status=True)
    ]
    dim_objs = [
        create_obj_struct(f"Btn{ep}_DimSw", "Switch", "1 Bit", "DPST-1-1", _dim_ui),
        create_obj_struct(f"Btn{ep}_DimBri", "Brightness", "1 Byte", "DPST-5-1", _dim_ui),
        create_obj_struct(f"Btn{ep}_DimStat", "Switch status", "1 Bit", "DPST-1-1", _dim_ui, is_status=True),
        create_obj_struct(f"Btn{ep}_DimBriS", "Brightness status", "1 Byte", "DPST-5-1", _dim_ui, is_status=True)
    ]
    cu_objs = [
        create_obj_struct(f"Btn{ep}_CuUpDn", "Up/down", "1 Bit", "DPST-1-8", _cu_ui),
        create_obj_struct(f"Btn{ep}_CuStop", "Stop", "1 Bit", "DPST-1-7", _cu_ui),
        create_obj_struct(f"Btn{ep}_CuPos", "Position", "1 Byte", "DPST-5-1", _cu_ui),
        create_obj_struct(f"Btn{ep}_CuUpDnS", "Up/down status", "1 Bit", "DPST-1-11", _cu_ui, is_status=True),
        create_obj_struct(f"Btn{ep}_CuPosS", "Position status", "1 Byte", "DPST-5-1", _cu_ui, is_status=True)
    ]
    fan_objs = [
        create_obj_struct(f"Btn{ep}_FanPow", "Power", "1 Bit", "DPST-1-1", _fan_ui),
        create_obj_struct(f"Btn{ep}_FanSpd", "Fan speed", "1 Byte", "DPST-5-10", _fan_ui),
        create_obj_struct(f"Btn{ep}_FanPowS", "Power status", "1 Bit", "DPST-1-1", _fan_ui, is_status=True),
        create_obj_struct(f"Btn{ep}_FanSpdS", "Fan speed status", "1 Byte", "DPST-5-10", _fan_ui, is_status=True)
    ]
    ac_objs = [
        create_obj_struct(f"Btn{ep}_AcPow", "Power", "1 Bit", "DPST-1-1", _ac_ui),
        create_obj_struct(f"Btn{ep}_AcTar", "Setpoint", "2 Bytes", "DPST-9-1", _ac_ui),
        create_obj_struct(f"Btn{ep}_AcMod", "Mode", "1 Byte", "DPST-20-105", _ac_ui),
        create_obj_struct(f"Btn{ep}_AcFan", "Fan speed", "1 Byte", "DPST-5-10", _ac_ui),
        create_obj_struct(f"Btn{ep}_AcSwi", "Air swing", "1 Bit", "DPST-1-1", _ac_ui),
        create_obj_struct(f"Btn{ep}_AcVan", "Louver position", "1 Byte", "DPST-5-10", _ac_ui),
        create_obj_struct(f"Btn{ep}_AcPowS", "Power status", "1 Bit", "DPST-1-1", _ac_ui, is_status=True),
        create_obj_struct(f"Btn{ep}_AcTarS", "Setpoint status", "2 Bytes", "DPST-9-1", _ac_ui, is_status=True),
        create_obj_struct(f"Btn{ep}_AcModS", "Mode status", "1 Byte", "DPST-20-105", _ac_ui, is_status=True),
        create_obj_struct(f"Btn{ep}_AcFanS", "Fan speed status", "1 Byte", "DPST-5-10", _ac_ui, is_status=True),
        create_obj_struct(f"Btn{ep}_AcSwiS", "Air swing status", "1 Bit", "DPST-1-1", _ac_ui, is_status=True),
        create_obj_struct(f"Btn{ep}_AcVanS", "Louver position status", "1 Byte", "DPST-5-10", _ac_ui, is_status=True)
    ]
    he_objs = [
        create_obj_struct(f"Btn{ep}_HePow", "Power", "1 Bit", "DPST-1-1", _he_ui),
        create_obj_struct(f"Btn{ep}_HeTar", "Setpoint", "2 Bytes", "DPST-9-1", _he_ui),
        create_obj_struct(f"Btn{ep}_HeMode", "Heating mode", "1 Bit", "DPST-1-100", _he_ui),
        create_obj_struct(f"Btn{ep}_HePowS", "Power status", "1 Bit", "DPST-1-1", _he_ui, is_status=True),
        create_obj_struct(f"Btn{ep}_HeTarS", "Setpoint status", "2 Bytes", "DPST-9-1", _he_ui, is_status=True),
        create_obj_struct(f"Btn{ep}_HeModeS", "Heating mode status", "1 Bit", "DPST-1-100", _he_ui, is_status=True),
        create_obj_struct(f"Btn{ep}_HeRmT", "Room temperature", "2 Bytes", "DPST-9-1", _he_ui, is_status=True)
    ]
    scene_ep_objs = [
        create_obj_struct(f"Btn{ep}_ScTrig", "Scene trigger", "1 Byte", "DPST-18-1", _sc_ui),
    ]
    ...
    endpoints_logic.append({
        ...
        "ac": {"rid_step": rid_ac_step, "rid_min": rid_ac_min_temp, "rid_max": rid_ac_max_temp, "rid_en_auto": rid_ac_en_auto, "rid_en_heat": rid_ac_en_heat, "rid_en_cool": rid_ac_en_cool, "rid_en_dry": rid_ac_en_dry, "rid_en_fan": rid_ac_en_fan, "rid_fan_auto": rid_ac_fan_auto, "rid_fan_type": rid_ac_fan_type, "objs": ac_objs},
        "he": {"rid_step": rid_he_step, "rid_min": rid_he_min_temp, "rid_max": rid_he_max_temp, "objs": he_objs},
        "scene_ep": {"objs": scene_ep_objs},
    })
...
for cfg in endpoints_logic:
    ...
    cfg["scene_ep"]["orids"] = register_objects(cfg["scene_ep"]["objs"])


def render_scene_ep_block(cfg, indent):
    """Render Scene function settings for an endpoint."""
    I = " " * indent
    res = []
    res.append(f'{I}<ParameterSeparator Id="{new_pb_id()}" Text="Scene settings" UIHint="Headline" />')
    res.append(f'{I}<ParameterRefRef RefId="{_rid_sc_count}" />')
    # Use nested choose for scenes to avoid sequential evaluate issues
    cur_indent = indent
    for sc, rid_n, rid_i, rid_num, rid_vis in scene_defs:
        res.append(' ' * cur_indent + f'<choose ParamRefId="{_rid_sc_count}">')
        res.append(' ' * (cur_indent + 2) + f'<when test=">={sc}">')
        if sc > 1: res.append(' ' * (cur_indent + 4) + f'<ParameterSeparator Id="{new_pb_id()}" />')
        res.append(' ' * (cur_indent + 4) + f'<ParameterSeparator Id="{new_pb_id()}" Text="  Scene {sc}" UIHint="Headline" />')
        res.append(' ' * (cur_indent + 4) + f'<ParameterRefRef RefId="{rid_n}" />')
        res.append(' ' * (cur_indent + 4) + f'<ParameterRefRef RefId="{rid_i}" />')
        res.append(' ' * (cur_indent + 4) + f'<ParameterRefRef RefId="{rid_num}" />')
        res.append(' ' * (cur_indent + 4) + f'<ParameterRefRef RefId="{rid_vis}" />')
        cur_indent += 4
    
    # Close scene choose/when blocks
    for _ in range(len(scene_defs)):
        cur_indent -= 4
        res.append(' ' * (cur_indent + 2) + '</when>')
        res.append(' ' * cur_indent + '</choose>')
    for orid in cfg["scene_ep"]["orids"]:
        res.append(f'{I}<ComObjectRefRef RefId="{orid}" />')
    res.append(f'{I}<ParameterRefRef RefId="{cfg["rid_visible"]}" />')
    return res


def render_ep_func_content(cfg, func_val, indent):
    """
    Render content for ONE specific function value inside a ParameterBlock.
    Common parameters (Name, Function) are now handled by the caller.
    """
    I = " " * indent
    res = []
    if func_val == 0:   # Disabled
        pass
    elif func_val == 1: # Switch
        ...
        res.append(f'{I}<ParameterRefRef RefId="{cfg["rid_visible"]}" />')
    elif func_val == 6: # Air conditioner
        ...
        res.append(f'{I}<ParameterRefRef RefId="{cfg["rid_visible"]}" />')
    elif func_val == 7: # Heater
        res.append(f'{I}<ParameterSeparator Id="{new_pb_id()}" Text="Temperature settings" UIHint="Headline" />')
        res.append(f'{I}<ParameterRefRef RefId="{cfg["he"]["rid_step"]}" />')
        res.append(f'{I}<ParameterRefRef RefId="{cfg["he"]["rid_min"]}" />')
        res.append(f'{I}<ParameterRefRef RefId="{cfg["he"]["rid_max"]}" />')
        for orid in cfg["he"]["orids"]: res.append(f'{I}<ComObjectRefRef RefId="{orid}" />')
        res.append(f'{I}<ParameterRefRef RefId="{cfg["rid_visible"]}" />')
    elif func_val == 8: # Scene control
        res.extend(render_scene_ep_block(cfg, indent))
    return res
def get_param_offset(internal_name):
    for p_str in parameters:
        if f'Name="{internal_name}"' in p_str:
            import re
            m = re.search(r'Offset="(\d+)"', p_str)
            if m:
...
# Rendering the 6 Endpoints sequentially without conditional wrapping to avoid ETS crash
for cfg in endpoints_logic:
    ep_n = cfg['ep']
    indent = 14
    inner_I = ' ' * indent
    L(inner_I + f'<ParameterBlock Id="{new_pb_id()}" Name="Endpoint {ep_n}" Text="Endpoint {ep_n}">')
    L(inner_I + f'  <ParameterRefRef RefId="{cfg["rid_name"]}" />')
    L(inner_I + f'  <ParameterRefRef RefId="{cfg["rid_func"]}" />')
    L(inner_I + f'  <choose ParamRefId="{cfg["rid_func"]}">')
    for func_val in range(1, 9):
        L(inner_I + f'    <when test="{func_val}">')
        for line in render_ep_func_content(cfg, func_val, indent + 6): L(line)
        L(inner_I + f'    </when>')
    L(inner_I + f'  </choose>')
    L(inner_I + f'</ParameterBlock>')
...
with open(output_path, "w", encoding="utf-8") as f:
    f.write("\n".join(lines) + "\n")
print(f"Generated knob_device.xml v20 (clean): Dynamic ep titles, function-specific GO names, no ComObject duplication.")
