...
# Endpoints no longer have "Scene". Renumbered 1-7.
add_pt_enum("EndpointFunction", "Endpoint Function", {1: "Switch", 2: "CCT", 3: "Dim", 4: "Curtain", 5: "Fan", 6: "AC", 7: "Thermostat"})
...
# New Switch Mode options based on knx_scene_4_button.xml
...
# Global Params
_rid_ep_count = add_param_basic("Gen_EpCount", "EndpointCount", "Endpoint Count", "6", 1)[1]
...
o_gl_temp = add_com_object("Temperature", "Temperature", "2 Bytes", "DPST-9-1", "Temperature Sensor")[1]
o_gl_hum = add_com_object("Humidity", "Humidity", "2 Bytes", "DPST-9-7", "Humidity Sensor")[1]
...
for sc in range(1, 17):
    _, rid_n = add_param_basic(f"Sc{sc}_Name", "NameText", f"Scene {sc} Name", f"Scene {sc}", 14, is_text=True)
    _, rid_i = add_param_basic(f"Sc{sc}_Icon", "SceneIcon", f"Scene {sc} Icon", "1", 1)
    _, rid_num = add_param_basic(f"Sc{sc}_Number", "SceneNumber", f"Scene {sc} Number", f"{sc}", 1)
    scene_defs.append((rid_n, rid_i, rid_num))

endpoints_logic = []
for ep in range(1, 7):
    _, rid_name = add_param_basic(f"Btn{ep}_Name", "NameText", "Name String", f"Ep{ep}", 14, is_text=True)
    ...
    o_dim_sw = add_com_object(f"Btn{ep}_DimPow", "Power", "1 Bit", "DPST-1-1", f"Endpoint {ep} - ...")
    o_dim_sw_s = add_com_object(f"Btn{ep}_DimPowS", "Status Power", "1 Bit", "DPST-1-1", f"Endpoint {ep} - ...", p_read="Enabled", p_write="Disabled", p_trans="Enabled", p_update="Enabled")
    o_dim_br = add_com_object(f"Btn{ep}_DimBri", "Brightness", "1 Byte", "DPST-5-1", f"Endpoint {ep} - ...")
    o_dim_br_s = add_com_object(f"Btn{ep}_DimBriS", "Status Brightness", "1 Byte", "DPST-5-1", f"Endpoint {ep} - ...", p_read="Enabled", p_write="Disabled", p_trans="Enabled", p_update="Enabled")
    ...
    # 5. Fan
    _, rid_fan_icon = add_param_basic(f"Btn{ep}_FanIcon", "FanIcon", "Icon", "1", 1)
    ...
    # 6. AC
    _, rid_ac_icon = add_param_basic(f"Btn{ep}_AcIcon", "ACIcon", "Icon", "1", 1)
    ...
    # 7. Thermostat
    _, rid_th_icon = add_param_basic(f"Btn{ep}_ThIcon", "ThermostatIcon", "Icon", "1", 1)
...
print("Generated knob_device.xml with refactored naming and endpoints")
