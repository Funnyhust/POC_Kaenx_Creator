...
for ep in range(1, 7):
    ...
    o_cu_updn = add_com_object(f"Btn{ep}_CuUpDn", "Up/Down", "1 Bit", "DPST-1-8", f"Endpoint {ep} - ...")
    o_cu_updns = add_com_object(f"Btn{ep}_CuUpDnS", "Up/Down status", "1 Bit", "DPST-1-11", f"Endpoint {ep} - ...", p_read="Enabled", p_write="Disabled", p_trans="Enabled", p_update="Enabled")
    o_cu_stop = add_com_object(f"Btn{ep}_CuStop", "Stop", "1 Bit", "DPST-1-7", f"Endpoint {ep} - ...")
    o_cu_pos = add_com_object(f"Btn{ep}_CuPos", "Position", "1 Byte", "DPST-5-1", f"Endpoint {ep} - ...")
    o_cu_poss = add_com_object(f"Btn{ep}_CuPosS", "Position status", "1 Byte", "DPST-5-1", f"Endpoint {ep} - ...", p_read="Enabled", p_write="Disabled", p_trans="Enabled", p_update="Enabled")
    ...
    endpoints_logic.append({
        ...
        "cu": {"rid_time": rid_cu_time, "rid_icon": rid_cu_icon, "orids": [o_cu_updn[1], o_cu_updns[1], o_cu_stop[1], o_cu_pos[1], o_cu_poss[1]]},
...
print("Generated knob_device.xml v9 with final General Settings and EP refinements")
