...
def add_param_basic(internal_name, pt_name, display_text, default_val, obj_size_bytes=1):
    global mem_offset
    pid, rid = new_p_id()
    # ALL parameters (including text) MUST have memory if they are part of the device state
...
for sc in range(1, 17):
    # Ensure default value fits in 14 bytes!
    _, rid_n = add_param_basic(f"Sc{sc}_Name", "NameText", "Name String", f"Scene {sc}", 14)
...
for ep in range(1, 7):
    ep_ui = f"Endpoint {ep} - ..."
    # Ensure default value fits in 14 bytes! "AC 0X" is safe. "Air cond 0X" is safe (11 chars).
    _, rid_name = add_param_basic(f"Btn{ep}_Name", "NameText", "Name String", f"Air cond 0{ep}", 14)
    ...
    sw_objs = [
        create_obj_struct(f"Btn{ep}_Switch", "Power", "1 Bit", "DPST-1-1", ep_ui),
        create_obj_struct(f"Btn{ep}_Stat", "Power Status", "1 Bit", "DPST-1-1", ep_ui, is_status=True)
    ]

    # 2. CCT
    _, rid_cct_icon = add_param_basic(f"Btn{ep}_CctIcon", "LightIcon", "Icon", "1", 1)
    cct_objs = [
        create_obj_struct(f"Btn{ep}_CctPow", "Power", "1 Bit", "DPST-1-1", ep_ui),
        ...
        create_obj_struct(f"Btn{ep}_CctPowS", "Power Status", "1 Bit", "DPST-1-1", ep_ui, is_status=True),
...
for sc, rid_n, rid_i, rid_num in scene_defs:
    L(f'                    <choose ParamRefId="{_rid_sc_count}" {"" if sc==1 else f"test=\">={sc}\""}>')
...
with open(output_path, "w", encoding="utf-8") as f:
    f.write("\n".join(lines) + "\n")
print("Generated knob_device.xml v12 with fixed Text memory and defaults")
